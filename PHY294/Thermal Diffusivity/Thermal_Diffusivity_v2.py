...
def find_sinusoid_phaseshifts(data):
    ...
    for col in data:
        ...
        print(overrawshift, phaseshift)
        print("over:", col, phaseshift, overphaseshift, overphaseshift-phaseshift)

        #errors[col] = np.sqrt(np.diag(covar))[0]
        errors[col] = abs(phaseshift-overphaseshift)

        over_sin_in_data = ([sinusoid_approx(t, phaseshift, 50, 50, 0, col) for t in time]) + ([None for i in range(len(list(data[col].index))-len(time))])
        sin_in_data = ([sinusoid_approx(t, overphaseshift, overrawshift[1], overrawshift[2], overrawshift[3], col) for t in time]) + ([None for i in range(len(list(data[col].index))-len(time))])
...
def dp_domega(omega, m, r_in, r_out):
    x = (omega / m) ** (1/2)
    return  dp_dz(omega, m, r_out) * ((1/2) * (omega ** (-1/2)) * (m ** (-1/2)) * r_out) -\
            dp_dz(omega, m, r_in) * ((1/2) * (omega ** (-1/2)) * (m ** (-1/2)) * r_in)

def dp_dr(omega, m, r):
    x = (omega / m) ** (1/2)
    return  dp_dz(omega, m, r) * x

def dm_domega(delta_phi, omega, r_in, r_out, d=0.001):
    m1 = solve_m(delta_phi, omega, r_in, r_out)
    m2 = solve_m(delta_phi, omega+d, r_in, r_out)
    return (m2 - m1) / d

def dr_in_domega(delta_phi, omega, r_in, r_out, d=0.001):
    m1 = solve_m(delta_phi, omega, r_in, r_out)
    m2 = solve_m(delta_phi, omega, r_in+d, r_out)
    return (m2 - m1) / d

def dr_out_domega(delta_phi, omega, r_in, r_out, d=0.001):
    m1 = solve_m(delta_phi, omega, r_in, r_out)
...
def solve_m_unc_old(omega, m, r_in, r_out, delta_phi_unc, omega_unc, r_in_unc, r_out_unc):
    print(delta_phi_unc ** 2, (dp_domega(omega, m, r_in, r_out) * omega_unc) ** 2, (dp_dr(omega, m, r_in) * r_in_unc) ** 2, (dp_dr(omega, m, r_out) * r_out_unc) ** 2)
    print(delta_phi_unc ** 2 - (dp_domega(omega, m, r_in, r_out) * omega_unc) ** 2 - (dp_dr(omega, m, r_in) * r_in_unc) ** 2 - (dp_dr(omega, m, r_out) * r_out_unc) ** 2)
    return ((delta_phi_unc ** 2 - (dp_domega(omega, m, r_in, r_out) * omega_unc) ** 2 - (dp_dr(omega, m, r_in) * r_in_unc) ** 2) ** (1/2)) / (dp_dm(omega, m, r_in, r_out))

def solve_m_unc(omega, m, r_in, r_out, delta_phi_unc, omega_unc, r_in_unc, r_out_unc):
    return (((1/dp_dm(omega, m, r_in, r_out)) * delta_phi_unc) ** 2 + (dm_domega(delta_phi, omega, r_in, r_out) * omega_unc) ** 2 + (dr_in_domega(delta_phi, omega, r_in, r_out) * r_in_unc) ** 2 + (dr_out_domega(delta_phi, omega, r_in, r_out) * r_out_unc) ** 2) ** (1/2)
...
data, phaseshifts, phaseshift_errors = find_sinusoid_phaseshifts(data)
...
for col in phaseshifts:
    ...
    period_unc = 1
    omega_unc = (((2 * np.pi) / ((int(col) * 10 * 2) ** 2)) * period_unc) / trials[col]
